...
bin/sa-digest-run.sh --dry-run              # write markdown to /tmp/team-digest-dry-runs/, skip Notion
...
For repeated debugging without spamming Notion, use `--dry-run` - the markdown lands in `/tmp/team-digest-dry-runs/sa-digest-<date>-v<N>.md` and you can `cat` or `diff` it before doing a real run. Files are ephemeral (cleared on reboot).
