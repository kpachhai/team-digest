...
Yes. Each person creates their own digest page in the shared Notion database. The "Auto-generated by" footer shows which instance created it. For automated runs, each person sets up their own routine/trigger.
...
- **Claude Code Desktop routine:** Delete the routine from Settings > Routines
- **Session-local cron:** Close the Claude Code session, or run `CronDelete` with the job ID
- **Remote trigger:** Use the RemoteTrigger tool with `action: "delete"` and your trigger ID
