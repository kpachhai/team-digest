...
Yes. Each person creates their own digest page in the shared Notion database. The "Auto-generated by" footer shows which instance created it. For automated runs, each person sets up their own launchd / cron schedule using `bin/sa-digest-run.sh` on their own machine.
...
- **macOS launchd:** `launchctl unload ~/Library/LaunchAgents/com.team-digest.sa-digest.plist` (and optionally delete the plist)
- **Linux cron:** `crontab -e` and remove or comment the `sa-digest-run.sh` line
