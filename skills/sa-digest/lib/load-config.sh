...
#   e.g. load-config.sh team-digest
#
...
if [ ! -f "$CONFIG_FILE" ]; then
  ...
  echo "       Run ./setup.sh from the team-digest repo, or '/team-digest setup' in Claude Code." >&2
  exit 1
fi
...
if missing:
    ...
    print(f'       Edit {config_file} or run "/team-digest setup".', file=sys.stderr)
    sys.exit(4)
