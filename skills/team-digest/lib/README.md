...
| `consolidate-matches.sh` _(iteration 5 - deterministic matches.json)_ | Merge per-strategy match-record JSONs from `$TEAM_DIGEST_MATCHES_DIR` (Mech A sidecars written automatically by `fetch-github-prs.sh` and `fetch-github-issues.sh` when the env var is set, plus Mech B / Strategy 2 / Strategy 3 outputs persisted by SKILL.md Phase 3d) into the canonical matches.json with MAX-confidence dedup on `(hip_id, repo, pr_number)`. Replaces the in-Claude-context Phase 3b merge that F4 surfaced as unreliable under high PR volume. | `<matches-dir> [<output-path>]` | Merged JSON array to stdout, or to `<output-path>` if given. |
