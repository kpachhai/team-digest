---
name: sa-weekly
description: SA Weekly Digest - synthesizes the past week (or any custom date range) of SA Daily Digests into a rollup summary, written to the same Notion database. Usage - /sa-weekly [YYYY-MM-DD | --from F --to T | --dry-run | config]
user-invocable: true
---
...
- `/sa-weekly --from 2026-04-25 --to 2026-05-03` - synthesize an arbitrary date range (inclusive on both ends). Useful for catching up after a missed week, post-conference recaps, sprint-aligned summaries, or any non-ISO-week window.
- `/sa-weekly --dry-run` - run the full pipeline but write the markdown to a local file instead of creating a Notion page
- `/sa-weekly 2026-05-07 --dry-run` - the ISO-week mode + dry run
...
The flags can appear in any order. Mixing a positional date with `--from`/`--to` is an error - pick one mode. `--from` and `--to` must appear together (specifying only one is an error). Dry-run output goes to `/tmp/team-digest-dry-runs/sa-weekly-<WEEK_LABEL>-v<N>.md`, ephemeral by design (compare once, discard). For custom ranges, `WEEK_LABEL` becomes `<from>_to_<to>` (e.g., `2026-04-25_to_2026-05-03`) so the file name stays informative.
...
eval "$(bash ~/.claude/skills/sa-weekly/lib/compute-week-window.sh)"
...
eval "$(bash ~/.claude/skills/sa-weekly/lib/compute-week-window.sh --from "$FROM" --to "$TO")"
```
