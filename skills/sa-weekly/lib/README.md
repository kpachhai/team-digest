...
Shell helpers invoked by the `sa-weekly` skill. The skill body owns
...
Same as `skills/sa-digest/lib/`: `set -euo pipefail`, positional args
...
## Sharing with sa-digest helpers

This skill ALSO calls `~/.claude/skills/sa-digest/lib/load-config.sh`
to read the shared `sa-digest` config. We deliberately read the same
config file rather than maintaining a separate `sa-weekly` config -
