...
/sa-digest          # Run in Claude Code - produces yesterday's digest
/sa-digest 2026-04-20  # Backfill a specific date
```
...
Skills read config from `~/.config/team-digest/config.json` at runtime, not from the repo. The config key must match the skill directory name (e.g., `sa-digest` skill reads `config["sa-digest"]`).
...
2. Copy `skills/sa-digest/SKILL.md` to `skills/<team>-digest/SKILL.md`, change all `sa-digest` references to `<team>-digest`
