...
- **Explicit scan window.** `/team-digest` scans a single day by default; pass a range (`2026-06-08..2026-06-14`, `--from/--to`, or `--days N`) to scan multiple days across all sources at once and write one page tagged with its `date:start..date:end` span. This is the token-efficient way to cover a week without running seven dailies. (Replaces the old always-on `pr_lookback_days` backfill, which silently turned every daily into a week-spanning scan.)
