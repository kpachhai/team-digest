...
│   ├── sa-digest-run.sh   # Daily - symlink onto $PATH for cron/launchd
│   └── sa-weekly-run.sh   # Weekly rollup
...
│   └── sa-weekly/         # Weekly rollup of daily digests
...
Pages you created                │   (sa-digest)         (one page per day,
...
Last 5-7 daily digests         ─┐
                                ...
                                │   (sa-weekly)        (one page per week,
                                                        Digest Type=Weekly)
```

## Skills shipped today

This repo ships two skills out of the box - one for each cadence the SA team uses:

- **`/sa-digest`** - the daily digest (the workhorse)
- **`/sa-weekly`** - the weekly rollup (synthesizes the past week's daily digests)

Both write to the SAME Notion database, distinguished by the `Digest Type` property (`Combined` vs. `Weekly`).

### What `/sa-digest` scans (six sources)

1. **GitHub Activity** - PRs, issues, and releases across configured GitHub orgs (currently `hiero-ledger`, `hashgraph`, `hedera-dev`). Priority repos get rich narrative summaries with synthesized themes, **SA Relevance** notes, and Mermaid diagrams for architectural changes; every other repo with activity in the date window gets a row in the **Other Active Repos** table.
2. **Industry News** - public RSS/Atom feeds (Hedera, Hashgraph, and Ethereum Foundation blogs) plus GitHub commit-watching for spec sets that don't publish RSS (EIPs). Configured via `rss_feeds` in `config.json`. Items dated to the digest day are grouped by category in an **Industry News** section.
3. **Notion Keyword Monitor** - searches the Notion workspace for pages **created** on the digest day matching configured keywords (EVM, smart contracts, HIP, etc.). One narrative summary per matched page with linked title, matched keywords, and SA relevance.
...
### What `/sa-weekly` does (cross-day synthesis, no re-scan)

`/sa-weekly` reads the past week's daily digests already in Notion (filtered by `Digest Type = Combined` and date in [Mon, Sun]) and synthesizes seven cross-day themes:
...
Critically, `/sa-weekly` does NOT re-scan GitHub, Notion, or RSS - the dailies have already done that work. The weekly is a pure synthesis layer over the daily output, which keeps token cost bounded and avoids drift between what each cadence "saw."
...
/sa-weekly 2026-05-07                                     # the week containing this date
/sa-weekly --from 2026-04-25 --to 2026-05-03              # arbitrary range (post-conf recap, missed-week catch-up, sprint window)
/sa-weekly --dry-run                                      # preview, no Notion write
/sa-weekly --from 2026-04-25 --to 2026-05-03 --dry-run    # custom range + dry run
```

Reads the past week's daily digests already in Notion (no re-scanning) and writes a synthesized weekly summary to the same database. Prerequisite: at least 5-7 dailies for the target week must already exist in Notion. See [docs/sa-weekly-quickstart.md](docs/sa-weekly-quickstart.md) for the full walkthrough.

**Run from the terminal (cron / launchd / scripts):**

```bash
bin/sa-digest-run.sh                       # yesterday's daily (writes to Notion)
bin/sa-digest-run.sh 2026-04-20 --dry-run  # specific daily, local file
bin/sa-weekly-run.sh                       # last full week's rollup
bin/sa-weekly-run.sh --dry-run             # preview the rollup, no Notion write
```
...
**Automate it:** See [docs/scheduling.md](docs/scheduling.md) for the launchd plist, cron syntax, and a GitHub Actions example. Recommended cadence: `bin/sa-digest-run.sh` every weekday morning, `bin/sa-weekly-run.sh` Monday morning after Friday's daily lands.
...
[1] notion-fetch the Notion config page (Keywords, Title Patterns,
    ...
    |          (Priority repos: narratives with Mermaid diagrams + SA Relevance.
...
[4.5] Pre-Write Link Audit (mandatory): every repo/PR/issue/release/
    ...
    |
    v
[5] notion-create-pages → SA Daily Digest database
    OR (if --dry-run) Write tool → /tmp/team-digest-dry-runs/sa-digest-<DATE>-v<N>.md
```
...
```
/sa-weekly [YYYY-MM-DD] [--dry-run]
    |
    v
[0] Argument parsing + load shared sa-digest config (no separate sa-weekly config)
    |
    v
[1] lib/compute-week-window.sh resolves to ISO week (Mon-Sun) timestamps,
    ...
    |
    v
[2] notion-query-data-sources on the SA Daily Digest database, filtered
...
[4] Synthesize 7 cross-day themes (no re-scanning of raw sources):
    ...
    |
    v
[4.5] Same Pre-Write Link Audit as /sa-digest
    |
    v
[5] notion-create-pages → same SA Daily Digest database with
    Digest Type = Weekly, date = WEEK_END (Sunday)
    OR (if --dry-run) Write → /tmp/team-digest-dry-runs/sa-weekly-<WEEK_LABEL>-v<N>.md
```
...
- [docs/sa-digest-quickstart.md](docs/sa-digest-quickstart.md) - 10-minute setup walkthrough for the daily digest
- [docs/sa-weekly-quickstart.md](docs/sa-weekly-quickstart.md) - The weekly rollup skill: prerequisites, usage, scheduling, failure modes
