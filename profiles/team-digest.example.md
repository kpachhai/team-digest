# Team Profile (worked example - Solutions Architect)

This is a worked example of a fully-fleshed-out team profile. The minimal placeholder template
at `profiles/team-digest.template.md` is what `setup.sh` copies on first run; this example shows
what a real profile looks like for a Solutions Architect team building on the Hedera/Hiero
ecosystem. Copy this file to `profiles/team-digest.md` and adjust if your team's shape is
similar - otherwise use the template and write your own.

> Both `team-digest.template.md` and `team-digest.example.md` are committed; only the
> personalized `team-digest.md` (gitignored) holds your live values.

---

## Role and Responsibilities

We are the Solutions Architect team. Our job is to design, validate, and guide
